    ...
    if (guiState.framebuffersWindowOpen) {
        if (igBegin("Framebuffers", &guiState.framebuffersWindowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize)) {
            for (int i = 0; i < MAX_FRAMEBUFFERS; i++) {
                igText("(valid: %d) (scaleToWindow: %d) (color: %u) (FBO: %u) (RBO: %u)", activeFramebuffers[i].valid, activeFramebuffers[i].scaleToWindow, activeFramebuffers[i].colorBuffer, activeFramebuffers[i].FBO, activeFramebuffers[i].depthBuffer);
            }
        }
        igEnd();
    }
