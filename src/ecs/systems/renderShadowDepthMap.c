    ...
    for (int i = 0; i < GET_SYSTEM_COMPONENTS_LENGTH(data, 0); i++) {
        ...
        for (int k = 0; k < GET_SYSTEM_COMPONENTS_LENGTH(data, 1); k++) {
            ...
            for (int j = 0; j < rendererData->opaqueMeshesLength; j++) {
                ...
                mesh->uniforms[mesh->uniformsLength++] = (struct uniformRenderData) {
                    .type = RENDERER_UNIFORM_MATRIX_4,
                    .location = glGetUniformLocation(mesh->shader.program, "lightProjection"),
                    .count = 1,
                    .data.matrix = lightProjection,
                };

                mesh->uniforms[mesh->uniformsLength++] = (struct uniformRenderData) {
                    .type = RENDERER_UNIFORM_MATRIX_4,
                    .location = glGetUniformLocation(mesh->shader.program, "lightView"),
                    .count = 1,
                    .data.matrix = lightView,
                };

                mesh->textures[mesh->texturesLength++] = (struct textureRenderData) {
                    .type = RENDERER_TEXTURE_2D,
                    .slot = GL_TEXTURE0 + TEXTURE_SHADOW,
                    .buffer = activeFramebuffers[light->shadowDepthMap].depthBuffer,
                };
